...
int main() {
    ...
    // Game selection — runtime in slot mode (set by platform's boot
    // scan over /scumm/*); compile-time in standalone (pre-set by
    // TSB_GAME_X in engine/src/game_table.cpp).
    const tsb::GameDescriptor *gd = tsb::g_current_game;
    if (!gd) {
        tsb::platform::no_data_splash("", false);
        while (1) tsb::platform::sleep_ms(500);
    }
    tsb::ScummEngine *eng = gd->create_engine(&osys);
