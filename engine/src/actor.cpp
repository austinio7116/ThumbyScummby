...
// ThumbyScummby transcription header — replaces the dozen scummvm-private
...
#if 0  // ThumbyScummby: ActorHE constructor — HE only.
...
#if 0  // ThumbyScummby: v0 branch — Actor_v0::walkBoxQueuePrepare disabled
	if (_vm->_game.version == 0) {
...
// gfx.cpp (VirtScreen, Gdi).  Per TRANSCRIPTION_PLAN.md these light up
// at step 4 (costume.cpp) and step 12 (gfx.cpp).  Until then our legacy
// actor_render_all in src_old/costume.cpp drives rendering using the
...
void Actor::startAnimActor(int f) {
	...
	if (isInCurrentRoom() && _costume != 0) {
		...
		if (_vm->_game.version >= 3 && f == _initFrame) {
			_cost.reset();
#if 0  // ThumbyScummby: HE-only branch — heversion is always 0 for v0-v5.
...
// ThumbyScummby: getActorName depends on object name table + _language +
// MKTAG-based resource lookup.  Light up at step 8 (object.cpp) for the
// rtActorName slot lookup; v0/v3-name tables follow when those games land.
//
// Empty stub satisfies callers (Actor::actorTalk uses it for log messages).
const byte *Actor::getActorName() {
    return (const byte *)"";
}
#if 0  // Original getActorName — restore at step 8.
const byte *Actor::getActorName_DISABLED() {
...
// ThumbyScummby: AKOS-only palette remapping (uses MKTAG, AKPL/RGBS chunks).
// Light up at step 4 (costume.cpp) when AKOS support lands — until then v4
// games don't call these.
#if 0  // TODO: enable at step 4 (costume.cpp)
...
#endif  // remapActorPalette / remapActorPaletteColor — AKOS only
...
#if 0  // ThumbyScummby: ScummEngine_v60he — HE only.
