...
// scummvm/common/pack-start.h enables platform-specific struct packing
// pragmas. We rely on `__attribute__((packed))` instead — applied directly
// to the Box struct below via PACKED_STRUCT.
...
// pack-end.h removed — PACKED_STRUCT attribute applies per-struct.
...
// ThumbyScummby: ScummEngine_v6 derivative class not modelled (we target
// v0-v5).  Method elided.
#if 0
...
// ThumbyScummby: v0 (Maniac Mansion C64) walk pathing not modelled yet.
// Re-enable when ScummEngine_v0 / Actor_v0 are added.
#if 0
...
#endif  // v0 walkboxFindTarget / v3 findPathTowardsOld / getGates elided
