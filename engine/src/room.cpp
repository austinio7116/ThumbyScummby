...
void ScummEngine::startScene(int room, Actor *a, int objectNr) {
	...
	// THUMBY-PORT: preemptive purge before the new room loads.  Without
	// this the engine only culls reactively during the allocations the
	// new room triggers, so a heavy room can peak well above the
	// minHeapThreshold before any cull fires.  Locked resources (charset,
	// current scripts) survive; eviction stops naturally there.
